...
export function RuntimeAgentRunTranscript({
  ...
  return (
    <div className={cn('mobile-game-agent-run space-y-1', className)}>
      <button
        type="button"
        ...
        className="mobile-game-agent-run-toggle group flex min-h-8 w-full min-w-0 items-center gap-2 rounded-md border border-[#dddddd] bg-white px-2.5 py-1.5 text-left text-xs transition-colors hover:bg-[#f8fafc]"
      >
      ...
      {expanded && (
        <div id={detailsId} className="mobile-game-agent-run-details ml-5 border-l border-[#dddddd] pl-2 pt-1">
          <SdkAgentRunTranscript
            ...
            className="mobile-game-agent-transcript"
          />
        </div>
      )}
...
export function RuntimeThinkingBlock({ content, isStreaming, className }: RuntimeThinkingBlockProps) {
  ...
  return (
    <div className={cn('mobile-game-thinking-block space-y-1', className)}>
      <button
        type="button"
        aria-expanded={expanded}
        aria-controls={detailsId}
        ...
        className="mobile-game-thinking-toggle group flex min-h-8 w-full min-w-0 items-center gap-2 rounded-md border border-[#dddddd] bg-white px-2.5 py-1.5 text-left text-xs transition-colors hover:bg-[#f8fafc]"
      >
        <ChevronRight className={cn('size-3 shrink-0 text-[#777169] transition-transform', expanded && 'rotate-90')} />
        {isStreaming ? (
      ...
      {expanded && content && (
        <div id={detailsId} className="mobile-game-thinking-details ml-5 max-h-[220px] overflow-y-auto whitespace-pre-wrap border-l border-[#dddddd] pl-2 pt-1 text-xs leading-5 text-[#555]">
          {content}
          {isStreaming && (
