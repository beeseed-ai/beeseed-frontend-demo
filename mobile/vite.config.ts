...
const sdkAlias = {
  ...
  '@standard/runtime-agent-run-transcript': path.resolve(__dirname, 'src/runtime-agent-run-transcript.tsx'),
}
...
function mobileMessageInputOverlay(): Plugin {
  return {
    name: 'beeseed-mobile-message-input-overlay',
    enforce: 'pre',
    transform(code, id) {
      const normalizedId = id.split('?')[0].split(path.sep).join('/')
      if (!normalizedId.endsWith('/beeseed-sdk/src/components/chat/MessageInput.tsx')) return null
...
function mobileAgentRunTranscriptOverlay(): Plugin {
  return {
    name: 'beeseed-mobile-agent-run-transcript-overlay',
    enforce: 'pre',
    transform(code, id) {
      const normalizedId = id.split('?')[0].split(path.sep).join('/')
      ...
      const loopAvatar = `      <Avatar className="mt-0.5 size-9 shrink-0">
        {member?.avatar_url ? <AvatarImage src={member.avatar_url} /> : null}
        <AvatarFallback className="text-xs">AI</AvatarFallback>
      </Avatar>`
      const mobileLoopAvatar = `      <button
        type="button"
        className="mobile-game-chat-agent-avatar-button mt-0.5 shrink-0"
        title="查看 Agent 详情"
        onClick={() => window.dispatchEvent(new CustomEvent('beeseed-mobile-open-agent', { detail: { agentId: loop.agentId } }))}
      >
...
function mobileAgentAvatarOverlay(): Plugin {
  return {
    name: 'beeseed-mobile-agent-avatar-overlay',
    enforce: 'pre',
    transform(code, id) {
      const normalizedId = id.split('?')[0].split(path.sep).join('/')
  ...
  const canOpenAgentDetail = Boolean(message.isAgent && message.senderId)
  const openAgentDetail = useCallback(() => {
    if (!canOpenAgentDetail || !message.senderId) return
    window.dispatchEvent(new CustomEvent('beeseed-mobile-open-agent', { detail: { agentId: message.senderId } }))
  }, [canOpenAgentDetail, message.senderId])`

      const askAvatar = `        <div className="shrink-0 mt-0.5">
      ...
      const mobileAskAvatar = `        <button
          type="button"
          className="mobile-game-chat-agent-avatar-button shrink-0 mt-0.5"
          title={canOpenAgentDetail ? '查看 Agent 详情' : undefined}
          onClick={openAgentDetail}
          disabled={!canOpenAgentDetail}
        >
          {message.senderAvatarUrl ? (
            <img src={message.senderAvatarUrl} alt="" className="w-9 h-9 rounded-full object-cover" />
          ) : (
      ...
      const normalAvatar = `        <div className="shrink-0 mt-0.5">
          {message.senderAvatarUrl ? (
            <img src={message.senderAvatarUrl} alt="" className="w-9 h-9 rounded-full object-cover" />
          ) : (
            <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-medium" style={{ backgroundColor: avatarColor(senderLabel) }}>
      ...
      const mobileNormalAvatar = `        <button
          type="button"
          className="mobile-game-chat-agent-avatar-button shrink-0 mt-0.5"
          title={canOpenAgentDetail ? '查看 Agent 详情' : undefined}
          onClick={openAgentDetail}
...
export default defineConfig({
  plugins: [syncSkillIcons(), agentSkillsDialogOverlay(), storageUploadIntegrityOverlay(), taskCalendarDisplayOverlay(), skillShortcutMenuOverlay(), agentManageTabOverride(), mobileMessageInputOverlay(), mobileAgentRunTranscriptOverlay(), mobileAgentAvatarOverlay(), react(), tailwindcss()],
  define: {
